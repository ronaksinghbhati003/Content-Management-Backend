...
import { getOrRefreshAccessToken } from './platforms/youtube.adapter';
...
export class PublishController {
    ...
    youtubeStatus = asyncHandler(async (req: AuthRequest, res: Response) => {
        ...
        // A stored 'connected: true' flag only reflects that OAuth succeeded at
        // some point — it says nothing about whether the refresh token is still
        // valid *right now* (Google can silently revoke it, e.g. unverified/
        // Testing-mode apps expire it after a few days). Actually attempt a
        // refresh so this endpoint — and anything gating on it, like the
        // Publish modal's pre-flight check — catches a dead token before the
        // user hits Publish, not after.
        try {
            await getOrRefreshAccessToken(userId);
        } catch (err: any) {
            logger.warn(`[Publish] YouTube token validation failed for user ${userId}: ${err.message}`);
            res.status(200).json(ApiResponse.ok({
                connected: false,
                error: 'Your YouTube connection has expired — please reconnect.',
    ...
    // ── yt-dlp Cookies (per-user, improves clip download quality) ──────────────

    /**
     * @route POST /api/v1/publish/youtube/cookies
     * @desc Save this user's yt-dlp cookies.txt content (Netscape format)
     */
    saveYtDlpCookies = asyncHandler(async (req: AuthRequest, res: Response) => {
        const userId = req.users?._id as string;
        const { cookiesText } = req.body;

        if (!cookiesText || typeof cookiesText !== 'string' || cookiesText.trim().length < 10) {
            res.status(400).json(ApiResponse.error(400, 'cookiesText is required and must look like a cookies.txt file.'));
            return;
        }
        if (!cookiesText.includes('youtube.com') && !cookiesText.includes('.google.com')) {
            res.status(400).json(ApiResponse.error(400, 'This file does not look like a YouTube/Google cookies export.'));
            return;
        }

        await User.findByIdAndUpdate(userId, {
            ytDlpCookiesText: cookiesText,
            ytDlpCookiesUpdatedAt: new Date(),
        });

        res.status(200).json(ApiResponse.ok(null, 'Cookies saved. Higher-quality downloads will be used from now on.'));
    });

    /**
     * @route GET /api/v1/publish/youtube/cookies/status
     * @desc Whether this user has yt-dlp cookies configured
     */
    ytDlpCookiesStatus = asyncHandler(async (req: AuthRequest, res: Response) => {
        const userId = req.users?._id as string;
        const user = await User.findById(userId).select('+ytDlpCookiesText');

        res.status(200).json(ApiResponse.ok({
            configured: !!user?.ytDlpCookiesText,
            updatedAt: user?.ytDlpCookiesUpdatedAt || null,
        }, 'Cookies status retrieved'));
    });

    /**
     * @route DELETE /api/v1/publish/youtube/cookies
     * @desc Remove this user's saved yt-dlp cookies
     */
    clearYtDlpCookies = asyncHandler(async (req: AuthRequest, res: Response) => {
        const userId = req.users?._id as string;
        await User.findByIdAndUpdate(userId, {
            $unset: { ytDlpCookiesText: 1, ytDlpCookiesUpdatedAt: 1 },
        });
        res.status(200).json(ApiResponse.ok(null, 'Cookies removed'));
    });
